...
import { ProofModeRoom } from "@/components/proof/proof-mode-room";
import {
  BrainCircuit,
  Send,
  User,
...
export function AILearningRoom({ subject, concept }: AILearningRoomProps) {
  const [viewMode, setViewMode] = useState<"tutor" | "practice" | "proof">("tutor");
  ...
  if (viewMode === "practice") {
    return (
      <PracticeEngine
        ...
        onEnterProofMode={() => setViewMode("proof")}
      />
    );
  }

  // If in proof view mode, render ProofModeRoom
  if (viewMode === "proof") {
    return (
      <ProofModeRoom
        subject={subject}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="gap-1.5 text-xs border-amber-500/60 bg-amber-950/20 text-amber-300 hover:bg-amber-900/40"
          >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                onClick={() => setViewMode("practice")}
                className="gap-1.5 text-xs border-zinc-700 hover:bg-zinc-800 text-zinc-200"
              >
                <Lightbulb className="w-3.5 h-3.5 text-amber-400" />
                <span>Practice</span>
              </Button>
              <Button
                size="sm"
                onClick={() => setViewMode("proof")}
                className="gap-1.5 text-xs bg-amber-600 hover:bg-amber-500 text-white shadow-sm font-medium"
              >
            ...
            <div className="rounded-xl border border-border/60 bg-zinc-900/50 p-6 text-center space-y-3 my-8">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                ...
                <button
                  type="button"
                  onClick={() => setViewMode("proof")}
                  className="text-xs text-amber-300 bg-amber-950/40 hover:bg-amber-950/70 border border-amber-500/30 rounded-lg px-3 py-1.5 transition-colors text-left flex items-center gap-1.5 font-medium"
                >
                  <Lock className="w-3 h-3 text-amber-400" />
                  <span>Ready to Prove Mastery in Proof Mode?</span>
                </button>
              </div>
            </div>
          )}
